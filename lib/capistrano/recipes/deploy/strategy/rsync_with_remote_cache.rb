  ...
  module Deploy
    module Strategy
      class RsyncWithRemoteCache < Remote
        ...
        default_attribute :rsh_ext_option, nil
        ...
        def rsync_command_for(server)
          "rsync #{rsync_options} --rsh='#{rsh_option(server)}' #{local_cache_path}/ #{rsync_host(server)}:#{repository_cache_path}/"
        end

        def rsh_option(server)
          rsh_ext_option || "ssh -p #{ssh_port(server)}"
        end
